...
| 9 | WhatsApp is a first-class channel with prefilled contextual messages (car, dates, location, price). Online payment = Phase 2 (CMI / Payzone; Stripe is not available to Moroccan merchants — verified). | Your brief + payment research (section 9.6). |
...
| Online payment at launch (CMI) | Launch with "réservation confirmée par WhatsApp/téléphone, paiement à la prise en charge"; CMI/Payzone in Phase 2 | CMI needs a bank merchant contract (1–4 weeks, 2,500–4,000 MAD setup, 1.5–2.5% per transaction) → not free, not launch-blocking. |
...
4. **Confirmation** — customer: Nom, Prénom, Téléphone (E.164, WhatsApp default = same number, checkbox "c'est mon WhatsApp"), Email, Numéro de vol (only for airport), Message. Then `VOTRE RÉSERVATION` summary: car · dates · lieux · `Location 390 × 6 = 2 340` · `Options 300` · `Livraison 0` · `TOTAL 2 640 MAD` · `Caution 5 000 MAD (bloquée, restituée sous 7 jours)` (from settings) · payment: `● Paiement à la prise en charge (espèces / carte)` (Phase 2 adds `○ Paiement en ligne`). Consent checkbox (CNDP wording, unchecked by default) + Turnstile anti-spam. CTA `CONFIRMER LA RÉSERVATION`.
...
### 9.5 Payments (Phase 2, not free)
Stripe: **not available to Moroccan merchants** (not on Stripe's country list). CMI: reference gateway for Moroccan + international cards with 3-D Secure, via a bank merchant contract (≈ 1–4 weeks, 2 500–4 000 MAD setup, 1.5–2.5% per transaction, monthly fees 0–200 MAD). Payzone: online registration, 48–72 h, 2–3% per transaction. PayPal: receivable but withdrawals to Moroccan banks restricted. Plan: launch without online payment (deposit and payment at pickup, confirmed by staff); add CMI or Payzone as "Payer en ligne" for an optional prepayment once Diab Car signs a contract.
...
| **v1.1** | Compare (3 cars), Google review import, customer documents + reminders, finance basics, CMI/Payzone if contracted, WhatsApp templates if worth it | — | later |
...
| Hold length and payment timing | Lost bookings vs blocked cars | 10 min hold, staff confirmation SLA shown on the confirmation page — confirm the SLA |
